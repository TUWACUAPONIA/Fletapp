...
const RankingsView: React.FC = () => {
  ...
  const rankedDrivers = useMemo(() => {
    if (!context || !context.users) return [];

    const drivers = context.users.filter(u => u.role === UserRole.DRIVER) as Driver[];

    const driverStats = drivers.map(driver => {
      const completedTrips = context.trips.filter(
        trip => trip.driver_id === driver.id && [TripStatus.COMPLETED, TripStatus.PAID].includes(trip.status)
      );
      ...
      return { ...driver, totalTrips, totalKms, totalKgs, totalM3s };
    });
      ...
      <div className="space-y-4">
        {rankedDrivers.map((driver, index) => (
          <div key={driver.id} className="staggered-child" style={{animationDelay: `${0.4 + index * 0.05}s`}}>
            <Card 
              className={`transition-all duration-300 ${index < 3 ? 'border-amber-600/60' : ''}`}
            >
